 ...
 * networkContextLearning package.
 * 
...
public class PropsUtil {
    ...
    private static  Properties networkcontextlearningproperties;

    private static String NETWORK_CONTEXT_LEARNING_PROPERTIES_FILE;
    //private static final String NETWORK_CONTEXT_LEARNING_PROPERTIES_FILE= "/resources/networkContextLearning.properties";
    ...
    public  PropsUtil() {
        networkcontextlearningproperties = new Properties();
        InputStream is = null;
        try {
            String configFile = "networkContextLearning.conf";
            String path = this.getClass().getProtectionDomain().getCodeSource().getLocation().getPath();
            path = URLDecoder.decode(path, "UTF-8");
            NETWORK_CONTEXT_LEARNING_PROPERTIES_FILE = (new File(path).getParentFile().getPath()+File.separator+configFile).toString();
            File f = new File (NETWORK_CONTEXT_LEARNING_PROPERTIES_FILE);
            is = new FileInputStream(f);   
            //is = PropsUtil.class.getResourceAsStream(NETWORK_CONTEXT_LEARNING_PROPERTIES_FILE);
            // is = new FileInputStream(AIRS_PROPERTIES_FILE);
            networkcontextlearningproperties.load(is);
            NETWORK_CONTEXT_SANCP_SENSOR_PATH_VALUE = networkcontextlearningproperties.getProperty(NETWORK_CONTEXT_SANCP_SENSOR_PATH_PROP);
            NETWORK_CONTEXT_SANCP_CONFIG_FILE_PATH_VALUE = networkcontextlearningproperties.getProperty(NETWORK_CONTEXT_SANCP_CONFIG_FILE_PATH_PROP);
            NETWORK_CONTEXT_SANCP_LOG_FILE_PATH_VALUE = networkcontextlearningproperties.getProperty(NETWORK_CONTEXT_SANCP_LOG_FILE_PATH_PROP);
            ONTAIRS_DATABASE_NAME_VALUE = networkcontextlearningproperties.getProperty(ONTAIRS_DATABASE_NAME_PROP);
            MYSQL_CONNECTION_USERNAME_VALUE = networkcontextlearningproperties.getProperty(MYSQL_CONNECTION_USERNAME_PROP);
            MYSQL_CONNECTION_PASSWORD_VALUE = networkcontextlearningproperties.getProperty(MYSQL_CONNECTION_PASSWORD_PROP);
            MYSQL_SERVER_NAME_VALUE = networkcontextlearningproperties.getProperty(MYSQL_SERVER_NAME_PROP);
            NETWORK_CONTEXT_PROFILE_TABLE_NAME_VALUE = networkcontextlearningproperties.getProperty(NETWORK_CONTEXT_PROFILE_TABLE_NAME_PROP);
            NETWORK_ASSETS_TABLE_NAME_VALUE = networkcontextlearningproperties.getProperty(NETWORK_ASSETS_TABLE_NAME_PROP);
            
            validate();
        } catch (IOException e) {
            throw new RuntimeException("Could not read SystemContext module config. (File: " + NETWORK_CONTEXT_LEARNING_PROPERTIES_FILE + ")", e);
        } finally {
            if (null != is) {
                try {
                    is.close();
